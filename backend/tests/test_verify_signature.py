...
def _sign_by_hand(cover_short_by: int = 0, break_signature: bool = False) -> bytes:
    ...
    signed byte range, as a signature-wrapping attack would. ``break_signature``
    flips a bit of the signature value, the last thing in the CMS, so the
    signed bytes still match their digest but the signature no longer matches
    the certificate.
    """
...
def test_checking_never_imports_oscrypto(tmp_path):
    ...
    the route never asks for. The check runs in a fresh interpreter that
    imports nothing but the service, so no other import can hide a regression.
    """
    ...
    script = (
        ...
        "print('oscrypto' in sys.modules)\n"
    ).format(root=str(REPO_ROOT), pdf=str(signed))
    out = subprocess.run([sys.executable, "-c", script], capture_output=True, text=True, cwd=REPO_ROOT, timeout=120)
