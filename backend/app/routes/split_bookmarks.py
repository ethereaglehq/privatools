...
async def split_by_bookmarks(file: UploadFile = File(...)):
    ...
    try:
        ...
        output_path = await asyncio.to_thread(split_bookmarks_service.split_by_bookmarks, str(temp_path))
        stem = safe_stem(file.filename)
        cleanup = BackgroundTask(remove_files, str(temp_path), output_path)
        return FileResponse(
            path=output_path,
            filename=f"{stem}_bookmarks.zip",
        ...
        if "no bookmark" in msg or "no outline" in msg:
            raise HTTPException(
                status_code=400,
                detail="This PDF has no bookmarks to split by",
            ) from exc
        ...
        remove_files(*to_remove)
        logger.exception("Unexpected error in /split-by-bookmarks")
        msg = str(exc).lower()
        if "password" in msg or "encrypted" in msg:
            raise HTTPException(
                status_code=400,
