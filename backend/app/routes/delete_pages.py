...
from ..services import delete_pages_service
...
async def delete_pages(
    ...
    try:
        ...
        try:
            ...
            if "cannot delete all" in msg:
                raise HTTPException(
                    status_code=400,
                    detail="Cannot delete every page in the PDF",
                ) from exc
            raise HTTPException(status_code=400, detail=str(exc)) from exc
        except Exception as exc:
            # parse_page_ranges in the service uses int() which can ValueError;
            # any other parse-time failure surfaces here.
            if "invalid literal" in str(exc).lower() or isinstance(exc, ValueError):
                raise HTTPException(
                    status_code=400,
        ...
        return FileResponse(
            path=output_path,
            filename=f"{stem}_pages_deleted.pdf",
        ...
        remove_files(*to_remove)
        logger.exception("Unexpected error in /delete-pages")
        msg = str(exc).lower()
