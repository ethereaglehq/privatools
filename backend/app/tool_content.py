...
TOOL_HOWTO: dict[str, list[dict[str, str]]] = {
    ...
    "redact-pdf": [
        ...
        {"name": "Apply redactions and download", "text": "Click Redact. The text and image pixels under the boxes are removed from the file, not just covered, so removing a black box does not bring them back. The questions below say what redaction leaves in place."},
    ],
    "flatten-pdf": [
    ...
    "delete-annotations": [
        {"name": "Add your PDFs", "text": "Drop or select one or more PDFs, up to 25 at a time and up to 500 MB each. Each file is uploaded and cleaned separately, one after another, and the server has five minutes to clean each one once it has arrived."},
    ...
    "bates-remove": [
        ...
        {"name": 'Describe the stamp', "text": 'Give the prefix and any suffix used when the numbers were applied, so the tool matches those stamps and leaves real page content alone. Then text made of exactly that prefix and suffix around a number is removed where it lies within an inch of any edge of the page, and any found elsewhere on the pages is left in place and reported. The digit count matters only when prefix and suffix are both blank: then anything in the top or bottom inch of the page as you see it that looks like a number of at least that many digits (three at the fewest), with or without letters such as DOC- in front, is removed, and on a page turned a quarter, such a number running along the inch at either side.'},
        {"name": "Download the clean PDF", "text": "The matching stamps are removed. The page tells you if any it found could not be, such as a stamp drawn by an annotation or a form field, and what it left in place: with a prefix or suffix, every other match for it; with both blank, Bates-shaped numbers on a page turned a quarter that lie in the inch at either side, or where Bates Numbering puts its stamp on such a page. You download a new PDF; the original on your device is not changed."},
    ],
    "accessibility-check": [
...
TOOL_FAQ: dict[str, list[dict[str, str]]] = {
    ...
    "redact-pdf": [
        {"q": "Is redaction permanent and truly irreversible?", "a": "Yes for the text and images under the boxes. A character that a box covers, fully or in part, is removed whole; one the box only just touches may stay. The image pixels under a box are blanked, and a line or shape that lies entirely inside a box is removed. The file is then rewritten without the old objects, so what was removed cannot be recovered. Some things are left in place. A line or shape that reaches past a box stays in the file beneath it. Form fields, and comments such as notes, highlights and drawings, are not part of the page's content, so one that overlaps a box is kept and still shows on top of it: delete them, or flatten the PDF, before you redact. Copies of the same words elsewhere, such as a bookmark title, the document metadata or the replacement text an accessible PDF gives screen readers, survive too, so check those."},
        ...
        {"q": "Will the redacted PDF still be searchable for non-redacted text?", "a": "Yes. Text outside the boxes stays searchable, including the hidden text layer that OCR adds to a scan, and bookmarks are kept. Three things go beyond a box: a character it covers only in part is removed whole, a link whose clickable area overlaps it is removed, and so is a typed text-box comment that overlaps it."},
        {"q": "Is metadata also redacted?", "a": "No. The boxes act on the page only. Author name, title, software, and other XMP/Info metadata are NOT automatically stripped — use the Strip Metadata tool afterward. Smart Redact leaves metadata alone too."},
    ...
    "delete-pages": [
        ...
        {"q": "Is this the same as Extract Pages?", "a": "It is the reverse. Delete Pages removes the pages you list; Extract Pages keeps only the pages you list. Both produce a new PDF and leave the original alone. The pages left out are not carried into the new file, and neither are the bookmarks and links that lead only to them or the comments, form fields and accessibility tags that belong only to them. A form field or tag shared with a page you keep stays, the tag without its alternate or replacement text. Delete Pages keeps the metadata, attachments, layers and scripts stored for the document as a whole, including whatever in them concerns a removed page: Sanitize Document removes those, and Strip Metadata the metadata alone. Of what is stored for the whole document, Extract Pages carries over only the title, language, display settings and accessibility tags."},
    ...
    "bates-remove": [
        ...
        {"q": "Does removing Bates numbers change the rest of the page?", "a": "A little. The stamps' text is removed, and text printed underneath a stamp loses the characters the stamp covers. A number printed into a scanned image is whitened in the picture along with its OCR text, and a link that lies on a stamp alone goes with it. Other text, pictures, drawings, links and comments stay. Taking a stamp out rewrites the page's content, and in some files that shifts other words on the page, so look the result over before you share it. The result is a new PDF, so the file on your device stays as it was."},
