...
TOOL_HOWTO: dict[str, list[dict[str, str]]] = {
    ...
    "env-validator": [
        ...
        {"name": "Read the validation report", "text": "The Validation report updates as you type and lists each finding with its line number, counting every line, including blank and comment lines. Errors are highlighted in red. When nothing is flagged, the report reads No obvious .env issues found."},
        {"name": "Fix the errors", "text": "An error means a line is not a usable KEY=value pair. A line with no equals sign gets missing =, and a name that is not ASCII letters, digits and underscores starting with a letter or underscore gets invalid variable name, as in the sample's Line 4: invalid variable name BAD NAME."},
        {"name": "Review the warnings", "text": "Warnings flag a key that is set again, an empty value, a value containing spaces or tabs that is not wrapped in matching quotes, and a short secret: a name containing SECRET, TOKEN, KEY or PASSWORD whose value is under 12 characters."},
    ...
    "delete-annotations": [
        ...
        {"name": "Download the result", "text": "A single file downloads automatically, as no_annotations.pdf or under its own name: contract.pdf comes back as no_annotations.pdf, but board_minutes.pdf keeps its name. Several files can be saved one by one or as delete-annotations-results.zip, where repeated names are numbered, as in no_annotations (2).pdf."},
    ],
    "delete-pages": [
    ...
    "odt-to-pdf": [
        ...
        {"name": "Know the default PDF settings", "text": "LibreOffice's standard PDF export settings apply. Headings become bookmarks, the PDF is tagged, which helps screen readers, and comments in the document are left out. Photos may be recompressed as JPEG at quality 90, and images sharper than 300 DPI are scaled down to 300 DPI. The result is not PDF/A."},
    ...
    "json-xml-formatter": [
        ...
        {"name": "Pick Format, Minify or Validate", "text": "Format adds line breaks and indentation: 2 spaces (the default), 4 spaces or Tabs. Minify removes the whitespace between tokens or tags. Validate checks the syntax and shows your text unchanged. The Indent menu only works with Format."},
    ...
    "video-resizer": [
        ...
        {"name": "Download the result", "text": "One video downloads as your file name with _resized.mp4, so clip.mov becomes clip_resized.mp4; several arrive together in media-results.zip. The index sits at the front of the MP4 (+faststart), so it can start playing online before it has fully downloaded."},
    ],
    "video-thumbnail": [
    ...
    "video-to-pdf": [
        ...
        {"name": "Choose the number of frames", "text": "Set the slider anywhere from 1 to 60; the default is 12. Each frame becomes one page, so this number is also the page count."},
        ...
        {"name": "Download the PDF", "text": "One video gives a PDF named after it with _frames.pdf, so lecture.mp4 becomes lecture_frames.pdf; several arrive together in media-results.zip."},
    ],
    "word-counter": [
        ...
        {"name": "Estimate the reading time", "text": "Reading time is the word count divided by 220 words per minute, rounded up to a whole minute, with at least 1 minute for any text: 220 words reads as 1 min and 221 words as 2 min."},
...
TOOL_FAQ: dict[str, list[dict[str, str]]] = {
    ...
    "json-to-yaml": [
        ...
        {"q": "Will it format the YAML correctly?", "a": "It writes valid YAML with 2-space indentation and block-style lists, and a YAML 1.2 parser reads the output back as the same data. One exception: a key spelled true, false or null (or yes, no, on or off for YAML 1.1 parsers such as PyYAML) is written without quotes, so a parser can read it as a boolean or null rather than text."},
    ...
    "sql-formatter": [
        {"q": "Which keywords start a new line?", "a": "These 23: SELECT, FROM, WHERE, GROUP BY, ORDER BY, HAVING, LIMIT, OFFSET, INNER JOIN, LEFT JOIN, RIGHT JOIN, FULL JOIN, CROSS JOIN, JOIN, ON, UNION, VALUES, INSERT INTO, UPDATE, SET, DELETE FROM, CREATE TABLE and ALTER TABLE. They match as whole words in any case and are uppercased. Other words, such as DISTINCT, AS, CASE, WITH, DESC or ALL, stay as you typed them."},
    ...
    "env-validator": [
        {"q": "What does the report say for the sample?", "a": "The sample has four lines: API_URL=https://privatools.me, SECRET_KEY=change-me, FEATURE_FLAG=true and BAD NAME=value. The copied report is two lines: [WARN] Line 2: SECRET_KEY looks short for a secret, because change-me has 9 characters, and [ERROR] Line 4: invalid variable name BAD NAME, because names cannot contain spaces."},
        ...
        {"q": "How are duplicate keys reported?", "a": "Each repeat gets a warning such as Line 2: duplicate key A, first seen on line 1. If a key appears three or more times, each warning names the previous line that set it rather than the first one."},
        {"q": "Are export lines and multi-line values supported?", "a": "No. export API_TOKEN=... is read as a variable named export API_TOKEN and reported as an invalid variable name. Each line is checked on its own, so a quoted value that continues over several lines, such as a PEM key, gets a missing = error for each continuation line."},
        ...
        {"q": "Will it reveal my secrets to PrivaTools?", "a": "No. Validation is plain JavaScript in the page; the tool makes no network request with the text and does not save it in browser storage. The report quotes only line numbers and the text before each =, so a copied report does not contain your values."},
    ],
    "json-to-csv-schema": [
    ...
    # ── Phase 7 — competitor-gap tools (v1.5.0) ──────────────────────────
    "mute-video": [
        {"q": "Is the picture changed at all?", "a": "No. The video stream is copied without re-encoding, so the resolution, frame rate, codec and quality stay as they were. Only the container is rewritten, without the audio."},
        ...
        {"q": "What limits apply?", "a": "Files over 200 MB are refused with \"File is too big for the server. Try compressing it first.\" FFmpeg has 180 seconds per file, after which the page says the server took too long, and the route is rate-limited at 5 requests a minute per IP address. Copying a stream needs no encoding, so it is much quicker than a conversion."},
    ...
    "odt-to-pdf": [
        ...
        {"q": "Are comments and form controls kept?", "a": "Comments are left out, and form controls such as check boxes and text fields are drawn as they would print rather than exported as fillable PDF fields; both follow LibreOffice's default export settings. Add fillable fields to the PDF afterwards with Form Creator if you need them."},
