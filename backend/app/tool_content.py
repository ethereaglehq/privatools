...
TOOL_FAQ: dict[str, list[dict[str, str]]] = {
    ...
    "image-upscaler": [
        ...
        {"q": "Why was my image rejected as too large?", "a": "Because the enlarged result would exceed the pixel limit the server enforces to protect its memory. Choose 2x instead of 4x, or reduce the source first with Resize and Crop Image."},
    ...
    "gif-to-jpg": [
        ...
        {"q": "What happens to a transparent GIF?", "a": "JPEG has no transparency. The alpha channel is discarded rather than blended onto a white page, so anything that was see-through takes the colour stored underneath it, which is usually black. Convert to PNG, WebP or TIFF if you need transparency kept."},
    ...
    "image-converter": [
        {"q": "Which formats can I convert between?", "a": "Anything the server's imaging library can read becomes JPG, PNG, WebP, BMP or TIFF. That covers the usual photo and graphic formats in both directions, so it doubles as a general repair tool for a file something else refuses to open."},
        ...
        {"q": "Are animations and multi-page files handled?", "a": "No. Only the first frame of an animated GIF or WebP, and only the first page of a multi-page TIFF, is converted. The remaining frames and pages are not included in the output."},
