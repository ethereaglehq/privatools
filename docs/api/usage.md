...
Before production enablement, run the job, cancellation, restart, expiry and
readiness tests; inspect the rendered docs; and verify the signed-image rollout
and rollback against the actual host. Measure workload latency and memory on
that host before publishing higher throughput guarantees. Local fixture
measurements validate behavior but are not Oracle capacity measurements.
