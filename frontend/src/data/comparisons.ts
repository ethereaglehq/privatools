  ...
  },
  {
    ...
    sections: [
      { heading: "Reading the Basic plan", body: [
        ...
        "PrivaTools applies the same limits to everyone. An upload request can be 500 MB, and most pages upload each file in a request of its own. Standard tool pages queue up to 25 files, but heavy tools accept about five runs a minute from one visitor, so in a long queue some files are marked failed and need a retry. A page keeps waiting while an upload is moving, but the server then gives each request five minutes and a PDF to Word conversion 60 seconds, so a very large or complex document can fail. There is no paid tier to lift any of this.",
      ] },
