...
type CatalogTool = { slug: string; name: string; description: string; category: string; accepts?: string; outputLabel?: string; byok?: boolean; clientOnly?: boolean; icon?: ElementType };
...
export function ToolWorkspace({ tool, categoryLabel, related, children, onFindTool }: {
    ...
    useEffect(() => {
        let active = true;
        import("@/data/tool-faq.json").then(module => { if (active) setQuestions((module.default as Record<string, { q: string; a: string }[]>)[tool.slug] || []); });
        return () => { active = false; };
    }, [tool.slug]);
    ...
    return <article className="tw-workspace" data-tool={tool.slug} style={{ "--tw-tone": `var(--pt-${tone})`, "--tw-tone-ink": `var(--pt-${tone}-ink)` } as CSSProperties}>
        ...
        <header className="tw-heading"><div className="tw-heading-copy"><h1>{tool.name}</h1><p className="tw-description">{tool.description}</p></div><span className="tw-tool-emblem" aria-hidden="true"><Icon size={48} strokeWidth={1.3} /><i /></span></header>
        ...
        {questions.length > 0 && <details className="tw-questions"><summary>Questions about {tool.name}<PlusMark /></summary><div>{questions.map(question => <section key={question.q}><h3>{question.q}</h3><p>{question.a}</p></section>)}</div></details>}
    </article>;
}
