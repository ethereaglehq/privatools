...
function ReviewNote({ date }: { date: string }) {
  return <p className="cmp-review-note"><BookOpen size={14}/><span>Reviewed <time dateTime={date}>{reviewDate(date)}</time> · By PrivaTools</span></p>;
}
...
export default function ComparePage({ competitorSlug }: { competitorSlug?: string } = {}) {
  ...
  useEffect(() => {
    document.title = comparison ? comparison.title : slug ? "Comparison not found · PrivaTools" : "Compare file tools and PDF workflows · PrivaTools";
  }, [comparison, slug]);
...
function ComparisonDirectory() {
  ...
  const filtered = useMemo(() => {
    ...
    return comparisons.filter(item => (category === "All products" || item.category === category) && (!search || `${item.name} ${item.category} ${item.summary}`.toLocaleLowerCase().includes(search))).sort((a, b) => sort === "category" ? a.category.localeCompare(b.category) || a.name.localeCompare(b.name) : a.name.localeCompare(b.name));
  }, [deferredQuery, category, sort]);
  return <StudioPage className="pt-comparison-page cmp-directory">
    <StudioHeader kicker="Product comparisons" title={<>A good fit for<br/><em>your kind of work.</em></>} description="Start with the task, then choose the tool. Explore practical differences between PrivaTools and the PDF and file products you’re considering." visual={<div className="cmp-directory-mark"><span>Different workflows.</span><strong>{comparisons.length}<i>alternatives</i></strong><p>One considered choice.</p></div>}/>
    <div className="cmp-directory-intro"><p>Desktop editing, quick conversions, forms, or your own server: each option has a place. Our comparisons use official product documentation and explain where either choice could suit you.</p><ReviewNote date={latestReview}/></div>
...
function ComparisonDetail({ comparison }: { comparison: ProductComparison }) {
  return <StudioPage className="pt-comparison-page cmp-detail">
    ...
    <div className="cmp-detail-reading">
      <aside className="cmp-reading-rail"><ReviewNote date={comparison.reviewedAt}/><p>Official product documentation.<br/>Practical editorial guidance.</p><nav aria-label="On this comparison"><a href="#comparison-choices" onClick={event => { event.preventDefault(); document.getElementById("comparison-choices")?.scrollIntoView({ block: "start" }); }}>Choose by workflow</a><a href="#comparison-details" onClick={event => { event.preventDefault(); document.getElementById("comparison-details")?.scrollIntoView({ block: "start" }); }}>The useful differences</a><a href="#comparison-sources" onClick={event => { event.preventDefault(); document.getElementById("comparison-sources")?.scrollIntoView({ block: "start" }); }}>Official sources</a></nav></aside>
      <div className="cmp-detail-content">
        <section className="cmp-answer" aria-labelledby="comparison-answer"><span className="cmp-eyebrow">Start with your workflow</span><h2 id="comparison-answer">Where each one fits.</h2><p className="comparison-summary">{comparison.summary} <SourcesInline comparison={comparison}/></p></section>
        ...
        <section className="cmp-choices" id="comparison-choices" aria-label="Reasons to choose each product">
          ...
          <article className="cmp-choice cmp-choice-other"><span className="cmp-choice-caption">Another useful direction</span><h2>Consider {comparison.name}<br/><em>when…</em></h2><ul>{comparison.chooseCompetitor.map(text => <li key={text}><ArrowUpRight size={16}/><span>{text}</span></li>)}</ul><SourcesInline comparison={comparison}/></article>
        </section>
        <section className="cmp-details" id="comparison-details" aria-labelledby="comparison-details-title"><div className="cmp-section-intro"><span className="cmp-eyebrow">Side by side</span><h2 id="comparison-details-title">The useful differences.</h2></div><div className="cmp-detail-rows">{comparison.features.map(feature => <article className="cmp-detail-row" key={feature.label}><h3>{feature.label}</h3><div><span className="cmp-row-product">PrivaTools</span><p>{feature.privatools}</p></div><div><span className="cmp-row-product">{comparison.name}</span><p>{feature.competitor} <SourcesInline comparison={comparison} only={feature.sourceUrl}/></p></div></article>)}</div></section>
        <section className="cmp-tradeoffs" aria-labelledby="comparison-tradeoffs-title"><span className="cmp-eyebrow">Before you decide</span><h2 id="comparison-tradeoffs-title">A few details to keep in view.</h2><ul>{comparison.tradeoffs.map((text, index) => <li key={text}><span className="cmp-tradeoff-number" aria-hidden="true">{String(index + 1).padStart(2, "0")}</span><p>{text} <SourcesInline comparison={comparison}/></p></li>)}</ul></section>
        <section className="cmp-sources" id="comparison-sources" aria-labelledby="comparison-sources-title"><div className="cmp-section-intro"><span className="cmp-eyebrow">The reading behind this page</span><h2 id="comparison-sources-title">Official sources.</h2></div><p>Reviewed on <time dateTime={comparison.reviewedAt}>{reviewDate(comparison.reviewedAt)}</time>. These are product descriptions, not independent tests. Choose-when notes are our editorial interpretation; pricing, access and performance were not benchmarked.</p><ol>{comparison.sources.map((source, index) => <li key={source.url}><span aria-hidden="true">{String(index + 1).padStart(2, "0")}</span><a href={source.url} target="_blank" rel="noreferrer"><strong>{source.label}</strong><small>{new URL(source.url).hostname}</small><ArrowUpRight size={18}/></a></li>)}</ol><Link to="/support" className="cmp-text-link">Spotted something that changed? Tell us <ArrowRight size={15}/></Link></section>
      </div>
    </div>
    <ProcessingNote/>
