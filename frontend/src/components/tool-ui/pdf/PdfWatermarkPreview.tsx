...
import { previewObjectUrl } from "../media/preview-url";
...
export function PdfWatermarkPreview({ file, mode, text, fontSize, opacity, position, image, imageScale }: {
    ...
    return <div className="pdf-stage-context"><PdfPageStage file={file} overlay={({ width, height }) => {
        ...
        if (imageMode) {
            ...
            const stamp = (px: number, py: number, key?: string) => <image key={key} href={previewObjectUrl(asset!.url)} x={px} y={py} width={w} height={h} opacity={opacity} />;
