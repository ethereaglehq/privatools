...
export function PdfPageStage({ file, page, onPageChange, regions = [], selectedId, onSelect, onDraw, drawLabel = "Draw a region", disabled = false, onDimensions, coordinateOrigin = "top", overlay }: {
    ...
    drawLabel?: string; disabled?: boolean; coordinateOrigin?: "top" | "bottom"; onDimensions?: (info: PdfPageDimensions) => void;
}) {
    ...
    useEffect(() => {
        ...
        setDocument(null); setLocalPage(1); setError(""); setRendering(true);
    ...
    useEffect(() => {
        ...
        document.getPage(activePage).then(async pdfPage => {
            ...
            const size = { width: native.width, height: native.height };
            setDimensions(size); dimensionCallback.current?.({ ...size, pages: document.numPages });
            const viewport = pdfPage.getViewport({ scale: Math.min(1.5, 900 / native.width) });
    ...
    const finish = (event: PointerEvent<SVGSVGElement>) => {
        ...
        const box = { x: Math.min(origin.x, end.x), y: Math.min(origin.y, end.y), width: Math.abs(end.x - origin.x), height: Math.abs(end.y - origin.y), page: activePage };
        if (box.width >= 2 && box.height >= 2 && !disabled) { onDraw?.(coordinateOrigin === "bottom" ? { ...box, y: dimensions.height - box.y - box.height } : box); setDrawing(false); }
    };
    ...
    return <section className="pdf-stage" aria-label="Your PDF preview">
        ...
        {error ? <div className="pdf-preview-error" role="alert"><strong>Preview unavailable</strong><p>{error}</p><p>Your source file is still selected. Unlock an encrypted PDF before editing it.</p></div> : <div className="pdf-stage-paper-area"><div className="pdf-stage-paper" style={{ aspectRatio: `${dimensions.width}/${dimensions.height}` }}>
            ...
            <svg viewBox={`0 0 ${dimensions.width} ${dimensions.height}`} className="pdf-region-layer" aria-label="Placed regions" data-drawing={drawing}
                ...
                {regions.filter(region => region.page === activePage).map(original => coordinateOrigin === "bottom" ? { ...original, y: dimensions.height - original.y - original.height } : original).map(region => <g key={region.id} tabIndex={onSelect ? 0 : undefined} role={onSelect ? "button" : undefined} aria-label={region.label || `Select region ${region.id}`} onClick={() => { if (!drawing) onSelect?.(region.id); }} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelect?.(region.id); } }}>
                    ...
                    {selectedId === region.id && <rect x={region.x} y={region.y} width={region.width} height={region.height} fill="none" stroke="#397dec" strokeWidth={2} strokeDasharray="5 3" />}
                </g>)}
                {overlay?.(dimensions)}
        ...
        <p className="pdf-stage-caption"><Hand size={14} />{drawing ? "Drag across the page to place a region. Exact coordinates remain editable." : `${Math.round(dimensions.width)} × ${Math.round(dimensions.height)} pt · Previewed on this device`}</p>
    </section>;
}
