...
export function CsvJsonUI() {
    ...
    const run = () => { resetResult(); try { setOutput(mode === "csv-to-json" ? csvToJson(input, delimiter) : jsonToCsv(input, delimiter)); emitToolRun({ outcome: "success" }); } catch (e) { setError(e instanceof Error ? e.message : "Couldn't convert this input."); emitToolRun({ outcome: "error" }); } };
