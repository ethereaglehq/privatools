...
- Public pages use Google Analytics, on by default, for page visits, sessions, engagement, where visits come from and tool runs: which tool, how it ran, how many files, whether it succeeded and, if it failed, a fixed failure category such as "too_large" or "server". Where a visit came from is the referring site's origin and any standard utm_ campaign tags, sent with the first page view only. It receives no file contents, filenames, document text, error messages or account identity; advertising features and Google Signals are off. Browsers that identify themselves as automated or headless are not measured. Visitors can turn analytics off on the privacy page.
