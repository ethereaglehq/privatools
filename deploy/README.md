...
- nginx's own error answers on the API host (413, 502, 503, 504) carry CORS headers for the allowed origins and a JSON detail, so the page can read them instead of reporting a network failure. The deploy never changes nginx: apply it by hand with [its runbook](api-subdomain-split.md#nginxs-own-error-answers-on-the-api-host-added-24-september-2026).
